...
def CurvsConvertToBezier(solid, angular_tol=0.01, linear_tol=0.01):
    """
    使用ShapeUpgrade_UnifySameDomain使形状的边界满足C1连续性。

    参数：
        solid (TopoDS_Solid): 输入的solid对象。
        angular_tol (float, optional): 角度容差，默认为0.01。
        linear_tol (float, optional): 线性容差，默认为0.01。

    返回：
        TopoDS_Solid: 处理后的solid对象，边界已满足C1连续性。
    """
    upgrader = ShapeUpgrade_ShapeConvertToBezier(solid.topods_shape())
    upgrader.Set2dConversion(True)
    upgrader.Set3dCircleConversion(True)
    upgrader.Set3dConicConversion(True)
    upgrader.Set3dConversion(True)
    upgrader.Set3dLineConversion(True)
    upgrader.SetBSplineMode(True)
    upgrader.SetExtrusionMode(False)
    upgrader.SetPlaneMode(False)
    upgrader.SetRevolutionMode(False)
    upgrader.SetSurfaceConversion(False)
    # print(upgrader.Get3dCircleConversion())
    # print(upgrader.Get2dConversion())
    # print(upgrader.Get3dConicConversion())
    # print(upgrader.GetSurfaceConversion())

    # 设置角度和线性容差
    # upgrader.SetAngularTolerance(angular_tol)   # 设置角度容差
    # upgrader.SetLinearTolerance(linear_tol)     # 设置线性容差
    # upgrader.UnifyEdges()
...
def unify_same_domain(solid, angular_tol=0.01, linear_tol=0.01):
    """
    使用ShapeUpgrade_UnifySameDomain使形状的边界满足C1连续性。
    ...
    返回：
        TopoDS_Solid: 处理后的solid对象，边界已满足C1连续性。
    """
    upgrader = ShapeUpgrade_UnifySameDomain(solid,True,False,True)
    upgrader.AllowInternalEdges(False)
    upgrader.SetSafeInputMode(True)
    # 设置修复选项
    upgrader.SetAngularTolerance(angular_tol)  # 设置角度容差
    upgrader.SetLinearTolerance(linear_tol)   # 设置线性容差
...
def extract_primitive(solid):
    ...
    """
    assert isinstance(solid, Solid),"solid type wrong  (extract_primitive)"
