...
def optimization_manager():
    ...
    def optimize_fn(optimizer, params, step, lr=1e-5,
                    warmup=50,
                    grad_clip=50):
...
def get_multi_sde_loss_fn(atom_sde, bond_sde, train, reduce_mean=True, continuous=True, eps=1e-5):
    ...
    def loss_fn(model, batch):
        ...
        atom_feat, atom_mask, bond_feat, bond_mask ,class_label= batch
        ...
        inputs=(perturbed_atom, perturbed_bond,t,atom_mask,bond_mask,class_label)
        atom_score, bond_score = score_fn(inputs)
...
def get_step_fn(sde, train, optimize_fn=None, reduce_mean=False, continuous=True, likelihood_weighting=False):
    ...
    def step_fn(state, batch):
        ...
        if train:
            ...
            optimizer.zero_grad()
            lossf,losse = loss_fn(model, batch)
            loss=lossf+losse
            loss.backward()
