...
class SurfPosNet(nn.Module):
    ...
    def __init__(self, use_cf):
        ...
        if self.use_cf:
            self.class_embed = Embedder(11, self.embed_dim) #cadnet50
...
class SurfNet(nn.Module):
    ...
    def __init__(self, use_cf):
        ...
        self.fc_out = nn.Sequential(
            ...
            nn.LayerNorm(self.embed_dim),
            nn.SiLU(),
            nn.Linear(self.embed_dim, 3*16+6),
        )
    ...
    def forward(self, surf, timesteps, class_label, is_train=False):
        """ forward pass """
        bsz = timesteps.size(0)
        surfZ=surf[...,:-6]
        surfPos=surf[...,-6:]
...
class FullNet(nn.Module):
    ...
    def __init__(self, use_cf):
        ...
        layer = nn.TransformerEncoderLayer(d_model=self.embed_dim, nhead=12, norm_first=True,
                                                   dim_feedforward=1024, dropout=0.1)
        self.net_edge = nn.TransformerEncoder(layer, 12, nn.LayerNorm(self.embed_dim))


        self.net_face = nn.TransformerEncoder(layer, 12, nn.LayerNorm(self.embed_dim))
        ...
        self.fc_out_edge = nn.Sequential(
            nn.Linear(self.embed_dim, self.embed_dim),
            nn.LayerNorm(self.embed_dim),
            nn.SiLU(),
            nn.Linear(self.embed_dim, 24),
        )
        self.fc_out_face = nn.Sequential(
            nn.Linear(self.embed_dim, self.embed_dim),
            nn.LayerNorm(self.embed_dim),
            nn.SiLU(),
            nn.Linear(self.embed_dim, 54),
        )
    ...
    def forward(self, edge,surf, timesteps, class_label, is_train=False):
        """ forward pass """
        bsz = timesteps.size(0)
        edgeZ, edgePos,vertPos = edge[:,:,:,:12], edge[:,:,:,12:18],edge[:,:,:,18:]
        surfZ,surfPos=surf[:,:,:-6],surf[:,:,-6:]
        edge_seq = edgePos.size(2)

        time_embeds = self.time_embed(sincos_embedding(timesteps, self.embed_dim)).unsqueeze(1) 
        surf_p_embeds = self.surfp_embed(surfPos)
        surf_z_embeds = self.surfz_embed(surfZ)
        repeat_surf_embeds = (surf_p_embeds + surf_z_embeds).unsqueeze(-2).repeat(1,1,edge_seq,1).flatten(1,2)

        edge_p_embeds = self.edgep_embed(edgePos)
        edge_z_embeds = self.edgez_embed(edgeZ)


        flatten_edge_embeds = (edge_p_embeds + edge_z_embeds).flatten(1,2)
        flatten_vert_pos_embeds = self.vertp_fc(vertPos).flatten(1,2)
        mean_edge_embeds = (edge_p_embeds + edge_z_embeds).mean(2)
        mean_vert_pos_embeds = self.vertp_fc(vertPos).mean(2)

        edge_data_embeds = repeat_surf_embeds + flatten_edge_embeds + flatten_vert_pos_embeds
        face_data_embeds = surf_p_embeds + surf_z_embeds + mean_edge_embeds + mean_vert_pos_embeds
        ...
        if self.use_cf:  # classifier-free
            if is_train:
                ...
                class_label[uncond_mask] = 0
            c_embeds = self.class_embed(class_label) 
            edge_tokens = edge_data_embeds + time_embeds + c_embeds
            face_tokens = face_data_embeds + time_embeds + c_embeds
        else:
            edge_tokens = edge_data_embeds + time_embeds
            face_tokens = face_data_embeds + time_embeds

        output_edge = self.net_edge(
            src=edge_tokens.permute(1,0,2)
        ).transpose(0,1)

        output_face = self.net_face(
            src=face_tokens.permute(1,0,2)
        ).transpose(0,1)
        
        pred_edge = self.fc_out_edge(output_edge).unflatten(1,torch.Size([edgePos.size(1), edge_seq]))
        pred_face = self.fc_out_face(output_face)
        return pred_edge,pred_face

class FullGeoNet(nn.Module):
    """
    Transformer-based latent diffusion model for edge latent z
    """
    def __init__(self, use_cf):
        super(FullGeoNet, self).__init__()
        self.embed_dim = 348
        self.use_cf = use_cf 
    ...
    def forward(self, edge,surf, timesteps, class_label, is_train=False):
        ...
        flatten_edge_embeds = (edge_p_embeds + edge_z_embeds).flatten(1,2)
        flatten_vert_pos_embeds = self.vertp_fc(vertPos).flatten(1,2)
        mean_edge_embeds = (edge_p_embeds** 2 + edge_z_embeds** 2).mean(2)
        mean_vert_pos_embeds = self.vertp_fc(vertPos** 2).mean(2)

        edge_data_embeds = repeat_surf_embeds + flatten_edge_embeds + flatten_vert_pos_embeds
        face_data_embeds = surf_p_embeds + surf_z_embeds + mean_edge_embeds + mean_vert_pos_embeds
        full_embeds=torch.cat([face_data_embeds,edge_data_embeds],dim=1)
        if self.use_cf:  # classifier-free
            if is_train:
                uncond_mask = torch.rand(bsz,1) <= 0.1  
                class_label[uncond_mask] = 0
            c_embeds = self.class_embed(class_label) 
            full_tokens = full_embeds + time_embeds + c_embeds
        else:
            full_tokens = full_embeds + time_embeds
        output_full = self.net(
            src=full_tokens.permute(1,0,2)
        ).transpose(0,1)
        output_edge=output_full[:,face_seq:,:]
        output_face=output_full[:,:face_seq,:]
