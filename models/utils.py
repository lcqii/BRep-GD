...
def get_model_fn(model, train=False):
    ...
    def model_fn(inputs, *args, **kwargs):
        ...
        """
        face_feat, edge_feat, timesteps, face_mask,edge_mask,class_label=inputs
        if not train:
            model.eval()
            return model(face_feat, edge_feat, timesteps, face_mask,edge_mask,class_label, *args, **kwargs)
        else:
            model.train()
            return model(face_feat, edge_feat, timesteps, face_mask,edge_mask,class_label, *args, **kwargs)
...
def get_multi_score_fn(sdes, model, train=False, continuous=False):
    ...
    if isinstance(sdes[0], sde_lib.VPSDE) or isinstance(sdes[0], sde_lib.subVPSDE):
        def score_fn(inputs, *args, **kwargs):
            perturbed_atom, perturbed_bond, t, face_mask, edge_mask,class_lable= inputs
            if continuous or isinstance(sde, sde_lib.subVPSDE):
                timesteps = t * 999
                inputs=(perturbed_atom, perturbed_bond, timesteps, face_mask,edge_mask,class_lable)
                face_score, edge_score= model_fn(inputs, *args, **kwargs)
                face_std = sde_face.marginal_prob(torch.zeros_like(face_score), t)[1]
                edge_std = sde_edge.marginal_prob(torch.zeros_like(edge_score), t)[1]
            else:
                assert False,"error"


            face_score = -face_score / face_std[:, None, None]
            edge_score = -edge_score / edge_std[:, None, None, None]
            return face_score, edge_score
...
def score_fn(out_score,t,x_sde):
    x_std = x_sde.marginal_prob(torch.zeros_like(out_score), t)[1]
    if len(out_score.shape)==3:
        x_score = -out_score / x_std[:, None, None]
    else:
        x_score = -out_score / x_std[:, None, None, None]
    return x_score
